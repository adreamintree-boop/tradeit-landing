...
import { useT, useLang, type MainDict } from "@/components/landing/i18n";
...
function HeroFloatingCards() {
  ...
  const cards: Array<Omit<FloatingCard, "floatClass" | "width" | "valueClass"> & { delay: string }> = [
    ...
    },
    {
      ...
      Icon: Weight,
      pos: "xl:right-[3%] 2xl:right-[20%] top-[50%] xl:top-[50%] 2xl:top-[50%] translate-y-[120px]",
      hideOn: "hidden xl:block",
      delay: "1.6s",
    },
  ];
  ...
  return (
    <section className="bg-white py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Mobile/Tablet: scroll-driven with sticky shared image */}
        <MobileProblems
          problems={PROBLEMS}
          displayIdx={displayIdx}
          setManualIdx={setManualIdx}
        />
      </div>
    </section>
  );
...
function MobileProblems({
  ...
  problems: { title: string; body: string; image: string; icon: React.ComponentType<{ className?: string }> }[];
  displayIdx: number;
...
function AICard({
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-3xl border border-border bg-white shadow-soft transition-all duration-300 hover:-translate-y-1 hover:shadow-card">
      <div className="aspect-square w-full overflow-hidden rounded-t-3xl bg-white p-[10px]">
        <div className="flex h-full w-full items-center justify-center overflow-hidden rounded-xl border border-border/40 bg-white">
          <img
            src={image}
            alt={title}
